        ...
        $.each(response.data, function () {
          ...
          if (Array.isArray(config.selectId)) {
            for (var i in config.selectId) {
              $("#" + config.selectId[i]).append(
                $("<option />").val(value).text(text)
              );
            }
          } else {
            $("#" + config.selectId).append(
              $("<option />").val(value).text(text)
            );
          }
          x++;
...
function deleteRecord(parameters) {
  ...
  Swal.fire({
    ...
    text: !parameters.text
      ? "Do you want to delete " + parameters.subject + "?"
      : parameters.text,
    icon: "question",
    showCancelButton: true,
    confirmButtonText: !parameters.confirmButtonText
      ? "Confirm Delete"
      : parameters.confirmButtonText,
  }).then((result) => {
...
function generateTableRowButtons(parameters) {
  ...
  if (parameters.view) {
    buttons +=
      "<a href='" +
      parameters.viewUrl +
      "' class='btn btn-sm btn-info' title='View More'><i class='fa fa-info-circle'></i></a> ";
  }

  if (parameters.edit) {
    buttons +=
      "<button type='button' class='btn btn-sm btn-warning btn-" +
      parameters.buttonFor +
      "-edit' ";
    buttons +=
      "data-id='" +
      parameters.rowId +
  ...
  if (parameters.delete) {
    buttons +=
      "<button type='button' class='btn btn-sm btn-danger btn-" +
      parameters.buttonFor +
      "-delete' ";
    buttons +=
      "data-id='" +
  ...
  parameters.forEach(function (parameter) {
    $(document).on("submit", parameter.formId, function (e) {
      ...
      if (parameter.createOrUpdate) {
        ...
        let baseUrl =
          "php-functions/" + parameter.moduleName + "-module-functions/";
